...
class BaseConsumer(JsonWebsocketConsumer):
    ...
    def message(self, event):
        ...
        logger.debug('Sending data: %s', 'the actual response as htmlpayload here')
    ...
    def reflex_message(self, data, **kwargs):
        ...
        try:
            ...
            # call the actial reflex method to make some custom actions within reflex:
    ...
    def render_page_and_broadcast_morph(self, reflex, selectors, data):
        html = self.render_page(reflex, data)
        if html:
            self.broadcast_morphs(selectors, data, html, reflex)

    def render_page(self, reflex, data):
        ...
        # Amended for wagtail-compatibility
        # Check if call made from wagtail page.
        if 'attrs' in data:
            dict = data['attrs']
            if 'data-wagtail' in dict:
                response = self.serve(reflex.request, reflex, reflex_context, *resolved.args, **resolved.kwargs)
            else:
    ...
    # Added for wagtail-compatibility, taken from 'wagtail.core.views'
    def serve(self, request, reflex, reflex_context, path):
        print('')
        ...
        template = page.get_template(request, *args, **kwargs)
        context.update(reflex_context)

        # taken from 'wagtail.core.models.serve'
    ...
    def delegate_call_to_reflex(self, reflex, method_name, arguments):
        ...
        # Call the actual reflex method to perform any actions:
        # in particular case we set the count variable.
